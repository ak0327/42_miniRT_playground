...
int	recursive_raytrace(const t_scene *scene, const t_ray *eye_ray, t_colorf *out_col, int recursion_level, t_img bump_img, t_img texture_img)
{
	...
	diffuse_reflect_color = calc_diffuse_reflection(scene, eye_ray,  intp, shape, bump_img, texture_img);
	color = colorf_add(color, diffuse_reflect_color);

	perfect_reflect_color = calc_perfect_reflection(scene, eye_ray, out_col, recursion_level, intp, shape, bump_img, texture_img);
	color = colorf_add(color, perfect_reflect_color);

	inflection_refraction_color = calc_inflection_refraction(scene, eye_ray, out_col, recursion_level, intp, shape, bump_img, texture_img);
...
{
	return (recursive_raytrace(scene, eye_ray, out_col, 0, bump_img, texture_img));
}
